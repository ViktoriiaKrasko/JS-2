...
function calculateExpression(expression) {
  return typeof expression === "string"
    ? eval(expression)
    : "Argument is not a string"; //Преревіряємо чи аргумент є рядком
...
function getNumberFromString(s) {
  let result = parseFloat(s); // Використовуємо функцію parseFloat для того, щоб спробувати отримати число з рядка
  if (isNaN(result)) {
...
function getIntegerFromString(s) {
  let result = parseInt(s); // Використовуємо функцію parseInt для того, щоб спробувати отримати ціле число з рядка
  if (isNaN(result)) {
...
function isNumberFinite(num) {
  ...
  return isFinite(num) ? "The number is finite." : "The number is not finite."; // Ця функція повертає true, якщо число є скінченним, і false, якщо число є нескінченним або не є числом.
...
function isValueNaN(value) {
  return isNaN(value) ? "The value is NaN." : "The value is not NaN."; // Використовуємо вбудовану функцію isNaN, щоб перевірити, чи є введене значення NaN.
...
function createURLObject(urlStr) {
  return new URL(urlStr); // Використовуємо вбудований конструктор URL, щоб створити новий об'єкт URL з введеного рядка.
...
function encodeURLComponent(urlComponent) {
  return encodeURIComponent(urlComponent); // Використовуємо вбудовану функцію encodeURIComponent, щоб закодувати введений компонент URL.
...
function encodeURL(url) {
  return encodeURI(url); // Використовуємо вбудовану функцію encodeURI, щоб закодувати введений URL.
...
function decodeURLComponent(urlComponent) {
  return decodeURIComponent(urlComponent); // Використовуємо вбудовану функцію decodeURIComponent, щоб декодувати введений закодований компонент URL.
